...
struct ReadRequest : Request {
    using Request::Request;
    virtual ResultType Process(const Database& db) const = 0;
};

struct GetBusInfoRequest : ReadRequest<BusInfoResponse> {
    GetBusInfoRequest() : ReadRequest(Type::GET_BUS_INFO) {}
    void ParseFrom(std::string_view input) override;
    BusInfoResponse Process(const Database& db) const override;
private:
    std::string id;
};
...
std::vector<BusInfoResponse> ProcessGetRequests(Database& db, const std::vector<RequestHolder>& requests);
